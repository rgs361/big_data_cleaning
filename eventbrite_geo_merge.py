"""
Script to merge geocoded lat/lng from Geocodio CSV back into eventbrite_events.json.
...
# Load the original eventbrite events from event_data folder
...
coords_map = {}
with open('event_data_geocodio/eventbrite_geocodio.csv', 'r', encoding='utf-8') as f:
...
with open('event_data_latlng/eventbrite_events_latlng.json', 'w', encoding='utf-8') as f:
    json.dump(events, f, indent=4, ensure_ascii=False)

print(f"✅ Created event_data_latlng/eventbrite_events_latlng.json")
print(f"   - {geocoded_count} events with coordinates")
print(f"   - {len(events) - geocoded_count} events without coordinates (invalid/skipped locations)")
